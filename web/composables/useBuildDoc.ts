...
export default function useBuildDoc() {
  ...
  if (!watcherInstalled) {
    ...
    watch(
      activeId,
      async (id) => {
        // Snapshot any pending writes for the previous project before we
        // wipe the live refs. We can't `await flushNow()` first — that
        // would leave the previous project's doc/title visible for the
        // duration of the IDB write, which is exactly the stale flash
        // we're avoiding.
        const pending =
        ...
        // Reset visible state synchronously so the UI never renders the
        // previous project's doc next to the new project's identity
        // while the next IDB read is in flight.
        if (timer) {
          clearTimeout(timer);
          timer = null;
        }
        ...
        // Trailing flush for the previous project runs in parallel with
        // the next load — they touch different records, so there's no
        // ordering hazard.
        ...
        // Stale-load guard: if the user switched projects again while
        // we were reading, drop this result — the next watcher
        ...
        if (existing) {
          ...
          // No record yet — seed the title with the project name. `doc`
          // stays at the EMPTY_DOC we already reset to above; the first
          // user edit triggers `schedule()` which writes the full record.
