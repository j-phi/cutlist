...
describe('Tidy Packer', () => {
  ...
  it('stacks same-width parts in a single column (rip-first)', () => {
    ...
    const bin = r(null, 0, 0, 10, 30);
    const result = packer.pack(
      bin,
      [r('a', 0, 0, 4, 6), r('b', 0, 0, 4, 6), r('c', 0, 0, 4, 6)],
      baseOptions,
    );
    expect(result.leftovers).toEqual([]);
  ...
  it('opens new strips for parts of different widths', () => {
    ...
    const result = packer.pack(
      bin,
      [r('wide', 0, 0, 4, 10), r('narrow', 0, 0, 2, 10)],
      baseOptions,
    );
  ...
  it('crosscut-first variant produces horizontal rows instead of columns', () => {
    ...
    const bin = r(null, 0, 0, 30, 10);
    const result = packer.pack(
      bin,
      [r('a', 0, 0, 6, 4), r('b', 0, 0, 6, 4), r('c', 0, 0, 6, 4)],
      baseOptions,
    );
    expect(result.placements).toHaveLength(3);
  ...
  it('reuses within-strip residuals for smaller parts', () => {
    ...
    const result = packer.pack(
      bin,
      [r('host', 0, 0, 6, 10), r('side-fit', 0, 0, 2, 6)],
      baseOptions,
    );
    expect(result.placements).toHaveLength(2);
  ...
  it('respects kerf gap between strips and stacked parts', () => {
    ...
    const result = packer.pack(
      bin,
      [r('a', 0, 0, 4, 4), r('b', 0, 0, 4, 4), r('c', 0, 0, 4, 4)],
  ...
  it('rotates rects to fit, preferring narrower columns', () => {
    ...
    const bin = r(null, 0, 0, 3, 6);
    const result = packer.pack(bin, [r('a', 0, 0, 6, 1)], {
      ...baseOptions,
      allowRotations: true,
    });
    expect(result.leftovers).toEqual([]);
  ...
  it('honours canRotateRect override (grain-locked parts stay oriented)', () => {
    ...
    const bin = r(null, 0, 0, 6, 6);
    const result = packer.pack(
      bin,
      [r({ id: 'a', locked: true }, 0, 0, 6, 3)],
      {
        ...baseOptions,
        allowRotations: true,
