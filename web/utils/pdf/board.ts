...
function drawBoardTilePage(
  ...
  for (const placement of layout.placements) {
    ...
    const pw = (((placement.rightM - placement.leftM) * 1000) / scale) * MM;
    const ph = (((placement.topM - placement.bottomM) * 1000) / scale) * MM;
    ...
    if (showPartNumbers) {
      ...
      const realCapM = Math.min(
        (placement.rightM - placement.leftM) / 2,
        ONE_INCH_M,
      );
